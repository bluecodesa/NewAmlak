        ...
        <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <div class="col-12 ">
                    <h4 class=""><a href="{{ route('Admin.home') }}" class="text-muted fw-light">@lang('dashboard') /</a>
                        <span class="text-muted fw-light"> @lang('Settings') / @lang('Privacy') & @lang('Terms') /
                            {{ __('Edit') . ' ' . __('Terms of advertising on the platform') }}</span>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('Admin.UpdateTermsAdvertising') }}" method="post" class="row">
                        @csrf
                        @foreach (config('translatable.locales') as $locale)
                            <div class="col-12 mb-3">
                                <label for="form-label mb-2">@lang('Terms') {{ __($locale) }}</label>
                                <textarea class="form-control textarea" name="{{ $locale }}[terms_advertising]" cols="100" rows="100" placeholder="">{!! $setting->translate($locale)->terms_advertising !!}</textarea>
                            </div>
                        @endforeach
