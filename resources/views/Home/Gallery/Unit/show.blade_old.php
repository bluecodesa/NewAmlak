...
<section id="gallery_unit_public" class="container">
    <div class="row show-unit">
        ...
        <div class="col-md-9">
            <div class="card border-0" style="border-radius: 28px;filter:drop-shadow(0px 3px 3px rgba(161,137,137,0.09 ));">
                ...
                <div class="card-body p-4 pt-3 pb-3">
                    ...
                    <div class="row">
                        <img src="{{ asset('dashboard/assets/new-icons/build.png') }}" style="width: 18px;height: fit-content;" class="p-0" />
                        <span class=" w-auto mb-2" style="color: #989898">
                    ...
                    <div class="row gallery-services mb-3" style="
            ...
            <div class="card border-0" style="border-radius: 20px;filter:drop-shadow(0px 3px 3px rgba(161,137,137,0.09 ));">
                <p class="font-weight-bold">
                    <img src="{{ asset('dashboard/assets/new-icons/Iconly_Light_Locatio.png') }}" style="width: 18px;height: fit-content;" class="p-0" />
                ...
                <div class="row m-0">
                    ...
                    <iframe width="100%" style="border-radius: 10px;" height="200" frameborder="0" style="border:0" src="https://www.google.com/maps/embed/v1/place?q={{ $Unit->lat_long }}&amp;key=AIzaSyAzFIgHaU5mzPcf16Qf3sdi0ioKqOKoy6E"></iframe>
        ...
        <div class="col-md-3 bg-white p-4" style="height: fit-content;border-radius: 20px;filter:drop-shadow(0px 3px 3px rgba(161,137,137,0.09 ));">
            ...
            @if ($broker->mobile)
            <a class="d-block btn-new text-center text-white" href="tel:{{ env('COUNTRY_CODE') . $broker->moblie }}" style="text-decoration: none;cursor: pointer;">{{ $broker->mobile }}</a>


            <a class="d-block btn btn-new-b mt-3 w-100" href="https://web.whatsapp.com/send?phone={{ env('COUNTRY_CODE') . $broker->mobile}}" target="_blank">دردشة</a>
            @endif
            <div class="row m-0 justify-content-between mt-3 align-items-center">

                <a class="love w-auto" data-toggle="modal" data-target="#interestUnit" href="" onclick="interestUnit({{ $Unit->id }})" style="color: #4a5865;border: 1px solid #eeecec; border-radius: 12px;"><img src="{{ asset('dashboard/assets/new-icons/interest.png') }}" class="img-fluid" style="height: 34px;" />تسجيل اهتمام</a>
                <a class="share w-auto p-0" data-toggle="modal" data-target="#shareLink">
                    <img src="{{ asset('dashboard/assets/new-icons/share.png') }}" class="img-fluid" style="height: 43px;border: 1px solid #eeecec;border-radius: 12px;cursor: pointer;" /></a>
            </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="shareLink" tabindex="-1" role="dialog" aria-labelledby="shareLinkTitle" aria-hidden="true">
                ...
                <div class="modal-body share-divs">
                    <div id="shareLink">
                        ...
                        <div class="row link justify-content-between">
                            <div class="w-auto" onclick="copy()" style="cursor: pointer"><svg xmlns="http://www.w3.org/2000/svg" width="20.782" height="30.633" viewBox="1039.055 450.797 19.891 24.817">
                                    <g data-name="copy">
                                        <path d="M1044.82 450.851c-.543.204-.941.558-1.18 1.049-.198.422-.237.975-.082 1.233.258.412.923.422 1.194.014.044-.068.093-.228.117-.354.044-.282.121-.418.3-.524.122-.068.685-.078 6.04-.078 6.51 0 6.068-.02 6.257.291.073.127.078.85.078 8.554 0 8.242 0 8.417-.097 8.568-.112.184-.214.242-.49.286-.433.063-.675.33-.675.738 0 .238.16.49.388.607.136.068.233.082.476.058a1.977 1.977 0 0 0 1.728-1.36c.073-.227.077-.96.068-8.98l-.015-8.738-.15-.315a2.059 2.059 0 0 0-.942-.942l-.316-.15-6.262-.01c-5.073-.005-6.296.005-6.437.053Z" fill="#497aac" fill-rule="evenodd" data-name="Path 29137" />
                                        <path d="M1040.616 455.152c-.694.141-1.262.65-1.49 1.335-.073.228-.077.961-.068 8.98l.015 8.739.15.315c.194.403.54.748.942.942l.316.15H1053.102l.315-.15c.403-.194.748-.539.942-.942l.15-.315.015-8.748c.01-8.68.01-8.748-.087-9.01-.214-.572-.612-.99-1.15-1.208l-.282-.112-6.092-.01c-3.35 0-6.185.015-6.297.034Zm12.238 1.471c.287.19.272-.335.272 8.777 0 7.505-.01 8.369-.077 8.514-.156.335.237.316-6.258.316-6.47 0-6.087.02-6.257-.306-.078-.146-.083-.781-.068-8.612l.015-8.451.116-.126a.73.73 0 0 1 .267-.17c.087-.03 2.67-.044 6-.039 5.583.01 5.86.015 5.99.097Z" fill="#497aac" fill-rule="evenodd" data-name="Path 29138" />
                                    </g>
                                </svg></div>
                            <input readonly class="w-75" style="text-align: left" id="share-url" value="{{ route('gallery.showUnitPublic', ['gallery_name' => $gallery->gallery_name, 'id' => $Unit->id])}} " />
                        </div>
                        ...
                        <div class="row justify-content-center p-4">
                            <div class="whats-conatiner">
                                {{-- whatsapp webbb --}}
                                <a href='https://web.whatsapp.com/send?text=Check%20out%20this%20link:%20{{ urlencode("$url") }}' target="_blank">
                                ...
                                {{-- - whatspp mob --}}
                                <a class="d-none" href="whatsapp://send?text=Check%20out%20this%20link:%20{{ urlencode('https://example.com') }}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="96px" height="96px" clip-rule="evenodd">
                                        <path fill="#fff" d="M4.868,43.303l2.694-9.835C5.9,30.59,5.026,27.324,5.027,23.979C5.032,13.514,13.548,5,24.014,5c5.079,0.002,9.845,1.979,13.43,5.566c3.584,3.588,5.558,8.356,5.556,13.428c-0.004,10.465-8.522,18.98-18.986,18.98c-0.001,0,0,0,0,0h-0.008c-3.177-0.001-6.3-0.798-9.073-2.311L4.868,43.303z" />
                                        <path fill="#fff" d="M4.868,43.803c-0.132,0-0.26-0.052-0.355-0.148c-0.125-0.127-0.174-0.312-0.127-0.483l2.639-9.636c-1.636-2.906-2.499-6.206-2.497-9.556C4.532,13.238,13.273,4.5,24.014,4.5c5.21,0.002,10.105,2.031,13.784,5.713c3.679,3.683,5.704,8.577,5.702,13.781c-0.004,10.741-8.746,19.48-19.486,19.48c-3.189-0.001-6.344-0.788-9.144-2.277l-9.875,2.589C4.953,43.798,4.911,43.803,4.868,43.803z" />
                                        <path fill="#cfd8dc" d="M24.014,5c5.079,0.002,9.845,1.979,13.43,5.566c3.584,3.588,5.558,8.356,5.556,13.428c-0.004,10.465-8.522,18.98-18.986,18.98h-0.008c-3.177-0.001-6.3-0.798-9.073-2.311L4.868,43.303l2.694-9.835C5.9,30.59,5.026,27.324,5.027,23.979C5.032,13.514,13.548,5,24.014,5 M24.014,42.974C24.014,42.974,24.014,42.974,24.014,42.974C24.014,42.974,24.014,42.974,24.014,42.974 M24.014,42.974C24.014,42.974,24.014,42.974,24.014,42.974C24.014,42.974,24.014,42.974,24.014,42.974 M24.014,4C24.014,4,24.014,4,24.014,4C12.998,4,4.032,12.962,4.027,23.979c-0.001,3.367,0.849,6.685,2.461,9.622l-2.585,9.439c-0.094,0.345,0.002,0.713,0.254,0.967c0.19,0.192,0.447,0.297,0.711,0.297c0.085,0,0.17-0.011,0.254-0.033l9.687-2.54c2.828,1.468,5.998,2.243,9.197,2.244c11.024,0,19.99-8.963,19.995-19.98c0.002-5.339-2.075-10.359-5.848-14.135C34.378,6.083,29.357,4.002,24.014,4L24.014,4z" />
    ...
    <div class="modal fade" id="interestUnit" tabindex="-1" role="dialog" aria-labelledby="interestUnitTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                ...
                                <div style="position:relative">
                                    <input type="tel" class="form-control" id="whatsapp" minlength="9" maxlength="9" pattern="[0-9]*" oninvalid="setCustomValidity('Please enter 9 numbers.')" onchange="try{setCustomValidity('')}catch(e){}" placeholder="599123456" name="whatsapp" required="" value="">

                                    <span style="position: absolute;left: -1px;top: 0;background-color: #e9ecef;height: 100%;display: flex; align-items: center;  justify-content: center;border-top-left-radius: 5px;border-bottom-left-radius: 5px;padding: 0px 20px;border: 1px solid #ced4da; border-top-left-radius: 8px;border-bottom-left-radius: 8px;">966+</span>
                                </div>
                            </div>
                        </div>
